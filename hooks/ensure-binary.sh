#!/usr/bin/env bash
#
# SessionStart hook: keep ${CLAUDE_PLUGIN_DATA}/bin/1nt in sync with the
...
#
# Any download actually happens via bin/1nt's own lazy path, so the worst
# case when this script fails is that the first `1nt` invocation of the
# session pays the download cost instead of the session start.

set -euo pipefail

plugin_root="${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT:-}}"
data_dir="${PLUGIN_DATA:-${CLAUDE_PLUGIN_DATA:-$HOME/.claude/plugins/data/1nitetent}}"
...
# If the user has a real `1nt` on PATH (cargo / brew / scoop / ...) we
# don't manage a plugin-owned copy — their install wins.
self_dir="$plugin_root/bin"
clean_path="$(echo "$PATH" | tr ':' '\n' | grep -vFx "$self_dir" | tr '\n' ':')"
if PATH="$clean_path" command -v 1nt >/dev/null 2>&1; then
  exit 0
fi
...
installed_version_file="$data_dir/bin/1nt.version"
installed_version="$(cat "$installed_version_file" 2>/dev/null || echo "")"
if [[ "$installed_version" == "$expected_version" ]]; then
  exit 0
fi

# Delegate the heavy lifting to bin/1nt so the download logic lives in one
# place. `1nt version` is harmless and exercises exactly the code path
# we want.
#
# Run detached in the background. Any network failure is logged but
# shouldn't prevent session start; bin/1nt will retry lazily on the first
# real invocation.
(
  if "$plugin_root/bin/1nt" version >/dev/null 2>&1; then
    :
  else
    echo "1nt: background install of v$expected_version failed; will retry on next invocation" >&2
  fi
) &
disown || true
